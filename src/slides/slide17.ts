...
/* SLIDE 17: WHAT I'D DO DIFFERENTLY — journal layout */
export function slide17() {
  ...
  ] as { num: string; t: string; d: string; highlight?: boolean }[];
  const items = lessons.map(l => `
    <div class="journal-entry${l.highlight ? ' highlight' : ''}">
      <div class="je-num">${l.num}</div>
      <div class="je-body">
        <div class="je-title">${l.t}</div>
        <div class="je-desc">${l.d}</div>
      </div>
    </div>
  `).join('');
  return `
    ${topbar(18, 'IF I COULD DO OVER')}
    <div class="frame journal-slide">
      <div class="anim-up">
      ...
      <div class="journal-entries anim-fade" style="animation-delay:0.3s">${items}</div>
    </div>
  `;
}
